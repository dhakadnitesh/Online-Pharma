...
function StomachCareUser({ ele }) {
  const navigate = useNavigate();

  function handleBtn1(id) {
    navigate(`/Stomach/Care/View/${id}`);
  }
  return (
    <>
      <Card id="card1">
...
export default function StomachCare() {
  ...
  async function StomachCareData(sortprice, searchprice) {
    setloading(true);
  ...
  useEffect(() => {
    StomachCareData(sortprice, searchprice);
    ...
    <>
      ...
      <Container maxWidth="85%" mt="3%" id="container">
        <SimpleGrid columns={{ base: 1, sm: 2, md: 3, lg: 4 }} spacing={10}>
          {StomachCare.map((ele, i) => (
            <StomachCareUser ele={ele} key={i} />
          ))}
        </SimpleGrid>
      </Container>
